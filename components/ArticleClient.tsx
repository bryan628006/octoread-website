...
import ArticleRiverChart from "@/components/ArticleRiverChart";
...
const articleData: Record<
  string,
  { topic: string; concept: string; articleName: string; title: string }
> = {
  "article-1": {
    topic: "Biology",
    concept: "Neural Plasticity",
    articleName: "Neuroplasticity and Cognitive Fitness",
    title: "Article 1: Neuroplasticity and Cognitive Fitness",
  },
};
...
function resolveSearchRoute(query: string): string {
  ...
  if (q.startsWith("article")) {
    return `/article/${q.replace(/\s+/g, "-")}`;
  }
  return `/concept/${q.replace(/\s+/g, "-")}`;
}
...
export default function ArticleClient({ slug }: ArticleClientProps) {
  ...
  const article = articleData[slug] ?? {
    topic: "Biology",
    concept: "Neural Plasticity",
    articleName: slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
    title: slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
  };

  function handleSearchKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter" && searchValue.trim()) {
      router.push(resolveSearchRoute(searchValue));
    }
  }
        ...
        <div className="flex flex-col gap-[24px]">
          ...
          <div className="flex flex-col gap-[4px]">
            <p className="text-[14px] font-bold text-[rgba(25,28,30,0.16)] leading-[1.571]">
              Topic: {article.topic} &nbsp;/&nbsp; Concept: {article.concept}{" "}
              &nbsp;/&nbsp; Article: {article.articleName}
            </p>
            <p className="text-[14px] font-semibold text-[#4b4b4a] leading-[1.571]">
              Snapshot of Article 1 Comprehension
            </p>
            <div className="flex items-center justify-between">
              <h1 className="text-[40px] font-bold text-[#56392f] leading-[1.6]">
                <span className="text-[#f5a623]">{article.articleName}</span> River
              </h1>

              {/* River / Ocean toggle */}
              <div className="bg-[#fac538] flex items-center gap-2 px-2 py-[6px] rounded-[15px] flex-shrink-0 ml-4">
                <button
          ...
          {/* Chart */}
          {activeView === "river" ? <ArticleRiverChart /> : <OceanChart view="ocean" />}
        </div>
      </main>
    </>
  );
